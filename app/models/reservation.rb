class Reservation < ActiveRecord::Base
  ...
  def archive
    ...
    Payment.find_all_by_reservation_id( self.id ).each {|p| p.update_attributes :credit_card_no => p.credit_card_no_obscured}
  ...
  def valid_dates?
    ...
    if unconfirmed_remote
      Blackout.all.each do |b|
        errors.add(:base, "dates #{b.startdate} to #{b.enddate} are blacked out, call for reservation") if startdate >= b.startdate and enddate <= b.enddate # 1
        errors.add(:base, "dates #{b.startdate} to #{b.enddate} are blacked out, call for reservation") if startdate <= b.startdate and enddate >= b.startdate # 2,4
        errors.add(:base, "dates #{b.startdate} to #{b.enddate} are blacked out, call for reservation") if startdate >= b.startdate and enddate == b.enddate # 3
      end
    end
