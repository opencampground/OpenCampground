...
class ApplicationController < ActionController::Base
  ...
  before_filter :need_ssl, :except => :ipn
  before_filter :set_cache_buster, :except => :ipn
  before_filter :set_locale, :except => :ipn
  before_filter :startup, :except => [:update_dates, :update_sitetype, :calendar_end_update, :calendar_update, :date_end_update, :date_update, :ipn]
  before_filter :display_control, :except => :ipn
  before_filter :set_current_user, :except => [:login, :initdemo, :find_remote, :create_remote, :ipn]
  # before_filter :authorize
  after_filter  :save_state, :except => [:partial_update, :destroy, :create, :calendar_update, :calendar_end_update, :date_end_update, :date_update, :update_week_year, :update_week_number, :update_week_count, :ipn]
  ...
  def date_update
    ...
    if params.key?(:month) && params[:month]
      month = params[:month].to_i
      begin
    ...
    if params.key?(:year) && params[:year]
      year = params[:year].to_i
      begin
  ...
  def startup
	...
	if db_current != DB_VERSION
	  ...
	  if Conflict::double_booking.size > 0
	    ...
	    flash[:error] = "reservation conflicts found!  Go to <a href=\"maintenance/conflicts\">Conflicts</a> to resolve."
